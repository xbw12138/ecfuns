...
public class AsyncTask_Search_User extends AsyncTask<String, String, String>{
	...
	private static String url_up = Config_mysql.mysql_url_search_user;
	Context context;
	...
	protected String doInBackground(String... p) {
	    ...
	    params.add(new BasicNameValuePair("user_phone", p[0]));
	       try{
	        ...
	        Singleton.getInstance().setHead(json.getString("user_head"));
	...
	@SuppressLint("ShowToast") protected void onPostExecute(String message) {      
		...
		if(message.equals("NONET")){
			...
			Toast.makeText(context, "获取信息成功", 8000).show(); 
		}else{
			Toast.makeText(context, "获取信息失败", 8000).show(); 
		}
    }
}
