...
import com.example.mysql.AsyncTask_Change_Pwd;
			...
			mSendmsgBtn.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(TextUtils.isEmpty(mUsernameET.getText().toString()))
					{
						...
						dialog=ProgressDialogs.createLoadingDialog(ForgetpwdActivity.this,"数据加载中");
						dialog.show();
						new Thread() {
							public void run() {
								try {
			...
			mSignupBtn.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					if (TextUtils.isEmpty(userName)) {
						...
						AsyncTask_Change_Pwd in=new AsyncTask_Change_Pwd(ForgetpwdActivity.this);
						in.execute(mUsernameET.getText().toString(),mPasswordET
								.getText().toString());
						finish();
						//Toast.makeText(ForgetpwdActivity.this, "暂未开放", 8000);
////////////////////////////////////////////////////////////////////////////////////
					}
				}
			});
			mSignupBtn.setClickable(false); 
			mSignupBtn.setBackground(ForgetpwdActivity.this.getResources().getDrawable(R.color.sign_up_btn_normal));
				...
				if (result == SMSSDK.RESULT_COMPLETE) {
					if (event == SMSSDK.EVENT_SUBMIT_VERIFICATION_CODE) {
						if (result==SMSSDK.RESULT_COMPLETE) {
							...
							mSignupBtn.setBackground(ForgetpwdActivity.this.getResources().getDrawable(R.color.sign_up_btn_press));
							mSignupBtn.setClickable(true); 
						}
					} else if (event == SMSSDK.EVENT_GET_VERIFICATION_CODE){
						Toast.makeText(getApplicationContext(), "验证码已经发送", Toast.LENGTH_SHORT).show();
						TimeCountUtil timeCountUtil= new TimeCountUtil(ForgetpwdActivity.this, 60000, 1000, mSendmsgBtn);
						timeCountUtil.start();
						content = new SmsContent(ForgetpwdActivity.this, new Handler(), mCodeET);
						   // 注册短信变化监听
				...
				public void afterTextChanged(Editable s) {
					if(mUsernameET.getText().length() == 11){
						if(judgephone(mUsernameET.getText().toString())){ 
							AsyncTask_Is_User_Exist change= new AsyncTask_Is_User_Exist(ForgetpwdActivity.this);
							change.setMysqlListenerss(new MysqlListenerss(){

								@Override
								public void Success() {
									// TODO 自动生成的方法存根
									Toast.makeText(getApplicationContext(), "等待发送验证码", 8000).show(); 
									mSendmsgBtn.setText(getString(R.string.register_send_verification_code));
									mSendmsgBtn.setClickable(true);
									mSendmsgBtn.setBackground(ForgetpwdActivity.this.getResources().getDrawable(R.drawable.btn_long_red));
								}

								@Override
								public void Fail() {
									// TODO 自动生成的方法存根
									Toast.makeText(getApplicationContext(), "账号不存，请您仔细想想", 8000).show(); 
									mSendmsgBtn.setText(getString(R.string.register_send_verification_code));
									mSendmsgBtn.setClickable(false);
									mSendmsgBtn.setBackground(ForgetpwdActivity.this.getResources().getDrawable(R.drawable.btn_long_white));
								}
								
							});
							change.execute(mUsernameET.getText().toString());
						}
						}else{
							success=false;
							mSignupBtn.setClickable(false); 
