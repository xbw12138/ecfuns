...
public class AsyncTask_Insert_Pl extends AsyncTask<String, String, String> {
	JSONParser jsonParser = new JSONParser();
	private static String url_up = Config_mysql.mysql_insert_pl;
	Context context;
	ProgressDialog dialog;

	public AsyncTask_Insert_Pl(Context context) {
	...
	protected String doInBackground(String... p) {
		...
		params.add(new BasicNameValuePair("pl_content", p[1]));
		params.add(new BasicNameValuePair("pl_num", p[2]));
		params.add(new BasicNameValuePair("pl_time", user_signtime));
		try {
